...
public class BreakHandler {
	...
	public static void onPlayerHarvestEvent(HarvestDropsEvent event) {
		...
		if (event.getHarvester() != null && Config.extraDropEnabled) {
			...
			try {
				if ((!Config.blacklistMode && Arrays.asList(Config.extraDropOres).contains(block.getRegistryName().toString())) || (Config.blacklistMode && !Arrays.asList(Config.extraDropOres).contains(block.getRegistryName().toString()))) {
					...
					for(ItemStack itemStack : event.getDrops()){
						if(!itemStack.getItem().getRegistryName().equals(block.getRegistryName()) && !(itemStack.getItem() instanceof ItemBlock)){
							...
							int multi = rand.nextInt(Config.maxDropMulti - Config.minDropMulti + 1) + Config.minDropMulti;
